...
def get_pdb_points(filename, atomarray):
    '''
    Inputs: 
    filename        <str> an alphafold pdb filename
    atomarray       list of <str> atoms to extract coords, ie ["N", "C", "CA"]
...
def pdb_take_photo(points_3d, colorlist, rvec, wiggle, atomarray):
    '''
    Inputs: 
    points_3d       <nd.array> (3,N) atomic xyz coordinates
    colorlist       <list> (3,N) B,G,R [0-255] resn color values per point_3d
    ...
    ### Plot 2D points as trangles per amino acid  
    img = np.zeros([image_height, image_width, 3], dtype=np.uint8)
    ptSize = 1  # Size of amino acid points
    fill = -1  # -1 = filled, else stroke width in px
    k = 0

    iterator = iter(points_2d.astype(int))
    while chunk := list(islice(iterator, len(atomarray))):
        pts = np.array(chunk, np.int32)
        pts = pts.reshape((-1,1,2))
        img = cv2.polylines(img, [pts], True, colorlist[k])
        k += 1
...
structure_id, points_3d, colorlist = get_pdb_points(filename, atomarray)
...
for round in tqdm(range(photos)):
    ...
    images = []
    for photo in r_vecs:
        images.append(pdb_take_photo(points_3d, colorlist, photo, wiggle, atomarray))
